...
public class EventControllerPrivate {
    ...
    public EventFullDto addEvent(@PathVariable Long userId, @RequestBody @Valid EventNewDto eventNewDto)
        ...
        log.info("Запрос POST / /users/{userId}/events / добавление пользователем {} нового события {}", userId, eventNewDto);
    ...
    public EventFullDto updateEventByOwner(@PathVariable Long userId,
            ...
            throws NotFoundException {
        log.info("Запрос PATCH / /users/{userId}/events/{eventId} / изменение события добавленного текущим пользователем {}", userId);
    ...
    public EventRequestStatusUpdateResult updateRequestsStatus(@PathVariable Long userId,
            ...
            throws NotFoundException {
        log.info("Запрос PATCH / /users/{userId}/events/{eventId}/requests / изменение статуса заявок на участие в событие");
    ...
    List<EventShortDto> getEventsByOwner(@PathVariable Long userId,
        ...
        log.info("Запрос GET / /users/{userId}/events / получение событий добавленных текущим пользователем {}", userId);
    ...
    public EventFullDto getEventByOwner(@PathVariable Long userId, @PathVariable Long eventId) throws NotFoundException {
        log.info("Запрос GET / /users/{userId}/events/{eventId} / получение информации о событие добавленом текущим" +
    ...
    public List<RequestDto> getRequestsByEventOwner(@PathVariable Long userId, @PathVariable Long eventId) throws NotFoundException {
        log.info("Запрос GET / /users/{userId}/events/{eventId}/requests / получение информации о запросах на участие" +
